...
//		}
	...
	content_slider.prototype = {
		...
		show_text: function (middle, dont_fadein, set_width) {
			...
			} else {//alert(this.new_width+'55');
			//alert(ww + ' '+ this.options.big_pic_width);
				if (this.minus>0) { 
		...
		_mouse_up : function (e) {
			...
			if (this.show_mouse_move) {	
				...
				if (Math.abs(this.math.sum_movement) == 0 && !this.was_gone) {
					...
					this.keep_going = 1;
					if (pos.pos < 0 ) { //alert(this.options.big_pic_width+' '+this.options.picture_class+222);
						this.click = 2;
					...
					if (pos.pos > 0) { 
					//alert(this.clicked +'===='+this.click);
					//alert($(this).attr('id'));
					
					//alert(this.options.big_pic_width+' '+this.options.picture_class+1111);
					 /* if ( this.options.picture_class === "circle_slider_thumb" ) { alert('****');
					addedClass = "green";
					//$("div").addClass("important");
					this.options.picture_class = 'hello';
		...
		_create_a_html_for_a_child : function (img,left) {
			...
			if (this.options.activate_border_div)
			{ 
			border_div='<div class="'+this.options.border_class+'"></div>';
			}
				
			  var str = '<div class="'+this.options.picture_class+'" style="left: '+left+'px;">'+border_div+'<img src="' + img + '"/></div>';
			  counter++;
			 return str;
		},
